...
func _process(delta: float) -> void:
	if melody.volume_linear != melody_volume_target:
		...
		var new_volume: float = melody.volume_linear
		new_volume = new_volume - speed * delta if melody_volume_target < melody.volume_linear else new_volume + speed * delta
		new_volume = clamp(new_volume, 0, 1)
		melody.volume_linear = new_volume
